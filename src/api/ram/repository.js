...
import { Ram } from './models/model.js';

export const getAllRepository = async (projections, populateProps) => {
    try {
        return await Ram.find().select(projections).populate(populateProps);
    } catch (err) {
...
export const updateRepository = async (id, body) => {
    try {
        return await Ram.updateOne({ _id: id }, body);
    } catch (err) {
        throw new RepositoryError(err.message, 500);
    }
};
